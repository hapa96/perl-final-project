...
my $EXPECTED =     {
      answered          => 0,
      name              => "test_data/exam_two_crosses.txt",
      result            => 0,
      total_questions   => 10,
    };
my $master_file = "test_data/valid_master_file_normal.txt";
my $exam_file = "test_data/exam_two_crosses.txt";
...
my $result =  correct_exams(master_exam => \%master_hash, student_exam => \%exam_hash, exam_name => $exam_file, total_questions => scalar $master_hash{Exam} -> {Questions} -> @*);
